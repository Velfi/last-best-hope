...
`build/last-best-hope --combat-stress`. It places 1,000 ships in 56 simulated
command elements: 320 friendly ships and 680 hostiles in layered formations
around the same seeded objective space. Individual ship marks establish battle
scale while orders, targeting, and damage remain formation-level, keeping
memory and simulation work bounded.

Use `build/last-best-hope --combat-stress-simulate 20 10000` for a repeatable
headless fixed-step CPU measurement of a 10,000-ship bounded stress state. The
first argument is fixed-step tick count and the second is exact ship count.

Launch the deterministic fleet finale with
`build/last-best-hope --combat-finale`. Its 44 command elements contain an
exact heap-backed roster of 2,200 ships: 60 coalition capital ships and 1,500
coalition strike craft against 40 defending capital ships and 600 defending
strike craft. Use `--capture-combat-finale <path>` for a diagnostic capture,
`--combat-finale-simulate <ticks>` for a short headless run, and
`--benchmark-combat-finale <samples> <seed>` for fixed-tick timing and roster
memory diagnostics.

Measure the rendered normal and 1,000-ship stress views with identical camera,
seed, resolution, warmup, and sample counts using:
...
`--capture-combat-late` to pin the pointer over Task Group 3 and include its
...
Run `build/last-best-hope --combat-simulate 100` for the deterministic balance
policy. The current 100-seed baseline delivers population in all 100 operations,
preserves all 7 command elements on average, and ends at 639.2 seconds on
average.
